...
def phi(g ,node ,l ,i):
    ...
    return min(g.nodes[node]['l' + str(l)]['k' + str(i)]['red']['val'],
               g.nodes[node]['l' + str(l)]['k' + str(i)]['blue']['val'])
...
def bestPartition(g, node, neighbour, l, i, col):
    ...
    for j in range(0, i + 1):
        ...
        tmp.append(
            g.nodes[node]['l' + str(l)]['k' + str()][col]['val'] + phi(g, neighbour, 1 if col == 'blue' else (l + 1),
                                                                       i - j))
    ...
    for i in range(0, len(tmp)):
        if tmp[i] == min(tmp):
            t.append(i)
    return [min(tmp), random.choice(t)]
...
def mCost(graph, l, i, Y, X, color):
    ...
    for j in range(0, i + 1):
        tmp.append(
            Y['l' + str(l)]['k' + str(i - j)][color] + X['l' + str(1 if color == 'blue' else (l + 1))]['k' + str(j)])
    # print("bestPartition: "+str(tmp))
    return min(tmp)
...
def nodeRun(graph, node, root, k, Avilabilty):
    ...
    p.reverse()
    depth = len(p) - 1
    att = {node: {'m' + str(c): {'l' + str(d): {'k' + str(j): {'red': 0, 'blue': 0} for j in range(0, k + 1)} for d in
                                 range(0, depth + 1)} for c in range(0, len(n))}}
        ...
        for neighbour in n:
            ...
            if first:
                for d in range(0, depth + 1):
                    ...
                    graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(0)]['red'] = \
                    graph.nodes[neighbour]['minSend']['l' + str(d + 1)]['k' + str(0)] + rate * load
                    ...
                    for i in range(1, k + 1):
                        ...
                        graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(i)]['red'] = \
                        graph.nodes[neighbour]['minSend']['l' + str(d + 1)]['k' + str(i)] + rate * load
                        if Avilabilty[int(node)]:
                            graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(i)]['blue'] = \
                            graph.nodes[neighbour]['minSend']['l' + str(1)]['k' + str(i - 1)] + rate
                        else:
                            graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(0)][
                                'blue'] = math.inf
                first = False
            else:
                ...
                for d in range(0, depth + 1):
                    ...
                    graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(0)]['red'] = mCost(graph,
                                                                                                                d, 0,
                                                                                                                PreviosYm,
                                                                                                                Xm,
                                                                                                                'red')
                    ...
                    for i in range(1, k + 1):
                        ...
                        Xm = graph.nodes[neighbour]['minSend']
                        graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(i)]['red'] = mCost(
                            graph, d, i, PreviosYm, Xm, 'red')
                        if Avilabilty[int(node)]:
                            graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(i)][
                                'blue'] = mCost(graph, d, i, PreviosYm, Xm, 'blue')  # hardFix
                        else:
                            graph.nodes[node]['m' + str(n.index(neighbour))]['l' + str(d)]['k' + str(i)][
                                'blue'] = math.inf
                ...
                # nx.set_node_attributes(graph,att)
        for d in range(0, depth + 1):
            graph.nodes[node]['minSend']['l' + str(d)]['k' + str(0)] = \
            graph.nodes[node]['m' + str(len(n) - 1)]['l' + str(d)]['k' + str(0)]['red']
            for i in range(1, k + 1):
                graph.nodes[node]['minSend']['l' + str(d)]['k' + str(i)] = min(
                    graph.nodes[node]['m' + str(len(n) - 1)]['l' + str(d)]['k' + str(i)]['red'],
                    graph.nodes[node]['m' + str(len(n) - 1)]['l' + str(d)]['k' + str(i)]['blue'])
...
def color(g, node, root, d, k):
    if node == root:
        for n in g.nodes:
            g.nodes[n]['color'] = 'red'
            # color(g, g.nodes[node]['children'][0], root, d + 1, k)
    ...
    if k > 0:
        ...
        if g.nodes[node]['m' + str(len(children) - 1)]['l' + str(d)]['k' + str(k)]['blue'] < \
                g.nodes[node]['m' + str(len(children) - 1)]['l' + str(d)]['k' + str(k)]['red']:
            g.nodes[node]['color'] = 'blue'
            d = 0
        if g.nodes[node]['m' + str(len(children) - 1)]['l' + str(d)]['k' + str(k)]['blue'] == \
                    g.nodes[node]['m' + str(len(children) - 1)]['l' + str(d)]['k' + str(k)]['red']:
            if random.choice(['red', 'blue']) == 'blue':
                g.nodes[node]['color'] = 'blue'
                d = 0
            # k=k-1
        for c in children[::-1]:
